...
#thing without the error checking
    with open (results_file, 'w') as outFile:   
        ...
        fig, axs = plt.subplots (2, 1, figsize = (12, 8))      #set up matplotlib figure and axes
        annualMeanTemp.plot (ax = axs [0])  #use pandas plot function to add figure
        axs [0].set (xlabel = "year", ylabel = "average temperature  [°C]",
                     title = "Annual average temperature at Durham") #add labels and title
        axs [0].get_legend().set_visible (False)  #turn off legend as it's not helpful
        annualRainfall.plot (ax = axs [1])  #use pandas plot function to add figure
        axs [1].set (xlabel = "year", ylabel = "annnual rainfall  [mm]",
                     title = "Annual rainfall at Durham") #add labels and title
        axs [1].get_legend().set_visible (False)  #turn off legend as it's not helpful
        plt.show ()   #force the figure to be output now
        fig.savefig ("DurhamAnnualTemperatureAndRainfall1850_2022.png")  #save the figure for future use
